...
esp_err_t SlaveSPI::begin(gpio_num_t so, gpio_num_t si, gpio_num_t sclk, gpio_num_t ss, size_t buffer_size, int (*callback)()) {
    ...
    tx_buffer       = (uint8_t *)heap_caps_malloc(max((int)max_buffer_size, 32),(int) SPI_MALLOC_CAP);
    rx_buffer       = (uint8_t *)heap_caps_malloc(max((int)max_buffer_size, 32),(int) SPI_MALLOC_CAP);
...
esp_err_t SlaveSPI::initTransmissionQueue() {
    ...
    int size = min((int)max_buffer_size, (int)output_stream.length());                  // NOT over the buffer's size.
